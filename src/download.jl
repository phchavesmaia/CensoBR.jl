...
function _censusurl(year::Integer, uf::Union{String,Symbol})
    ...
    if year == 2010 && uf == "SP"
        throw(
            ArgumentError(
                "Census 2010 SP is split into multiple archives; support for SP should be handled separately.",
            ),
        )
    end
...
function _downloadcensus(
    ...
    _downloadfile(
        ...
        description = "Downloading $uf $year",
    )
end
...
function _extractarchive(zip_path::AbstractString; force::Bool = false)
    ...
    run(
        pipeline(
            `$(p7zip_jll.p7zip()) x $zip_path -o$destination -y`,
            stdout = devnull,
            stderr = devnull,
        ),
    )
