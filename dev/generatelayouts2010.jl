...
const ODS_SHEETS = Dict(:household => "DOMI", :person => "PESS", :emigration => "EMIG", :mortality => "MORT")
...
function _toint(value; default=nothing)
  if isnothing(value) || ismissing(value) || (value isa AbstractString && isempty(strip(value)))
    isnothing(default) && throw(ArgumentError("Expected an integer value, found an empty cell"))
...
function _parsespreadsheetlayout(path::AbstractString, record::Symbol)
  ...
  size(data, 2) >= 12 ||
    error("Unexpected layout structure in sheet `$sheet`: " * "expected at least 12 columns, found $(size(data, 2))")

  fields = Vector{Dict{String,Any}}()

  for row in 3:size(data, 1)
    var = data[row, 1]
    ...
    description = (isnothing(description) || ismissing(description)) ? "" : String(description)
    ...
    if !ischaracter && intwidth + decimals != width
      error("Numeric width mismatch for `$name`: " * "positions imply width=$width, but INT+DEC=$(intwidth + decimals)")
    end
...
function _generatelayouts2010(documentationdir::AbstractString, outputdir::AbstractString)
  isdir(documentationdir) || throw(ArgumentError("Documentation directory does not exist: $documentationdir"))
  ...
  mktempdir() do tmpdir
    ...
    for record in (:household, :person, :emigration, :mortality)
      ...
      println("Generated $(basename(outputpath)): ", "$nfields fields, LRECL=$(layout["lrecl"])")
    end
  end
