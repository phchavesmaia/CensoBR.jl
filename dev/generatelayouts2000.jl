...
function _findfile(root::AbstractString, filename::AbstractString)
  ...
  length(matches) == 1 || error("Found multiple copies of `$filename` below `$root`: $(join(matches, ", "))")
...
function _parse_layout_metadata(text::AbstractString)
  ...
  isempty(lines) && return (label=nothing, values=Dict{String,String}(), notes=String[])
...
"""
	_parsesaslayout(path, record)

Parse an official Census 2000 SAS fixed-width input layout and return a
TOML-friendly dictionary.
"""
function _parsesaslayout(path::AbstractString, record::Symbol)
  isfile(path) || throw(ArgumentError("SAS layout file does not exist: $path"))
  ...
  field_regex = r"(?ms)^\s*@(\d+)\s+([A-Za-z0-9_]+)\s+(\$?)(\d+)\.(\d+)?\s*/\*(.*?)\*/"
  ...
  isempty(fields) && error("No fixed-width fields were parsed from `$path`")

  # Prefer the record length explicitly declared by SAS.
  lrecl_match = match(r"LRECL\s*=\s*(\d+)"i, text)
  inferred_lrecl = maximum(field["start"] + field["width"] - 1 for field in fields)
  lrecl = isnothing(lrecl_match) ? inferred_lrecl : parse(Int, lrecl_match.captures[1])

  inferred_lrecl > lrecl &&
    error("Parsed fields extend to column $inferred_lrecl, " * "beyond declared LRECL=$lrecl in `$path`")

  sort!(fields; by=field -> field["start"])

  Dict{String,Any}("record" => String(record), "lrecl" => lrecl, "source_file" => basename(path), "fields" => fields)
end

"""
	generatelayouts2000(documentation_dir, output_dir)
...
"""
function generatelayouts2000(documentation_dir::AbstractString, output_dir::AbstractString)
  isdir(documentation_dir) || throw(ArgumentError("Documentation directory does not exist: $documentation_dir"))
  ...
  for record in (:household, :family, :person)
    ...
    println("Generated $(basename(output_path)): ", "$nfields fields, LRECL=$(layout["lrecl"])")
  end
...
function main(args)
  length(args) <= 1 || error("Usage:\n" * "  julia --project=dev dev/generatelayouts2000.jl [outputdir]")
