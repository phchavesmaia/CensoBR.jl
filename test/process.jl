@testitem "Find raw Census files" begin
    ...
    mktempdir() do tmpdir
        ...
        @test basename(CensoBR._findrawfile(dir2010, household2010)) ==
              "Amostra_Domicilios_33.txt"

        @test basename(CensoBR._findrawfile(dir2010, person2010)) ==
              "Amostra_Pessoas_33.txt"

        @test basename(CensoBR._findrawfile(dir2010, emigration2010)) ==
              "Amostra_Emigracao_33.txt"

        @test basename(CensoBR._findrawfile(dir2010, mortality2010)) ==
...
@testitem "Parse fixed-width field" begin
    ...
    character =
        CensoBR.LayoutField("CODE", 1, 4, 0, true, nothing, Dict{String,String}(), String[])

    integer = CensoBR.LayoutField(
        "COUNT",
        5,
        4,
        0,
...
@testitem "Parse blank fixed-width fields" begin
    ...
    character =
        CensoBR.LayoutField("CODE", 1, 3, 0, true, nothing, Dict{String,String}(), String[])

    numeric = CensoBR.LayoutField(
        "VALUE",
        4,
        3,
        0,
...
@testitem "Census table schema" begin
    ...
    mktempdir() do tmpdir
        ...
        @test schema.types ==
              (Union{Missing,String}, Union{Missing,Int}, Union{Missing,Float64})
    end
end
...
@testitem "Parse Census file errors" begin
    ...
    mktempdir() do tmpdir
        @test_throws ArgumentError CensoBR._parsefile(
            joinpath(tmpdir, "missing.txt"),
            layout,
        )
    end
end
